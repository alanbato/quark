...
class Compiler:
    ...
    def define_function(self, func_name):
        self.current_function.append(func_name)
        if func_name in self.func_directory:
            Exception("Function {} already defined".format(ident))
        else:
            self.func_directory[func_name] = FuncRecord(None, {})
    ...
    def process_param(self, ident, type_):
        ...
        function.vars_[ident] = VarRecord(type_, 0, None)
    ...
    def get_variable(self, ident, scope=None):
        if scope is None:
            ...
            raise NotImplementedError('Supplying a specific context is not yet supported.')
        self.operand_stack.append(
    ...
    def handle_assignment(self, ident, type_):
        ...
        for type_, vars_ in var_ctx.items():
            if ident in vars_:
                raise Exception(f"Variable {ident} already defined with type {type_of_vars}")
        else:
